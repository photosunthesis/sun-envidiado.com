...
So I split that job out. Now there is a simple endpoint at `/api/blog-slugs.json` that returns nothing but the slugs. The newsletter reads that instead of the whole feed. The RSS feed stays exactly as it was, so anyone subscribed in their reader notices nothing.
